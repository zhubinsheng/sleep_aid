...
# 配置
SERVER_URL = "ws://47.110.80.169:8765"
SAMPLE_RATE = 24000  # 采样率 - 24kHz，与服务器一致
CHANNELS = 1  # 声道数 - 单声道，与服务器一致
...
class AudioPlayerRecorder:
    ...
    def add_audio_frame(self, audio_data):
        """添加音频帧到播放缓冲区和录制（兼容Opus编码和原始PCM）"""
        try:
            # 尝试解码Opus数据
            pcm_data = self.decoder.decode(audio_data, CHUNK)
            ...
            if self.frame_count % 200 == 0:
                print(f"已处理 {self.frame_count} 帧（Opus解码），播放缓冲区: {len(self.audio_buffer)} 帧")
                
        except Exception as e:
            # 如果Opus解码失败，说明是原始PCM数据，直接使用
            self.audio_buffer.append(audio_data)
            self.audio_data.append(audio_data)
            self.frame_count += 1
            if self.frame_count % 200 == 0:
                print(f"已处理 {self.frame_count} 帧（原始PCM），播放缓冲区: {len(self.audio_buffer)} 帧")
        ...
        async with websockets.connect(SERVER_URL) as websocket:
            ...
            while True:
                try:
                    ...
                    # 解码并播放Opus编码的音频数据
                    player_recorder.add_audio_frame(audio_frame)
